<template>
  <div class="pref-keybindings">
    ...
    <section class="keybindings">
      <div class="text">
        {{ $t('preferences.keybindings.customize') }}
      ...
      >
        ...
        <el-table-column prop="accelerator" :label="$t('preferences.keybindings.keyCombination')" width="220">
        </el-table-column>
        <el-table-column fixed="right" :label="$t('common.options')" width="90">
          <template slot-scope="scope">
            <el-button @click="handleEditClick(scope.$index, scope.row)" type="text" size="small" :title="$t('common.edit')">
              <i class="el-icon-edit"></i>
            </el-button>
            <el-button @click="handleResetClick(scope.$index, scope.row)" type="text" size="small" :title="$t('preferences.keybindings.reset')">
              <i class="el-icon-refresh-right"></i>
            </el-button>
            <el-button @click="handleUnbindClick(scope.$index, scope.row)" type="text" size="small" :title="$t('preferences.keybindings.unbind')">
    ...
    <section class="footer">
      <separator></separator>
      <el-button size="medium" @click="saveKeybindings">{{ $t('common.save') }}</el-button>
    ...
    <section v-if="showDebugTools" class="keyboard-debug">
      <separator></separator>
      <div><strong>{{ $t('preferences.keybindings.debug.title') }}</strong></div>
      <el-button size="medium" @click="dumpKeyboardInformation">{{ $t('preferences.keybindings.debug.dumpKeyboard') }}</el-button>
    </section>
    <key-input-dialog
...
export default {
  ...
  methods: {
      ...
      if (this.keybindingConfigurator && this.keybindingList.length > 0) {
        this.keybindingConfigurator.save()
          .then(success => {
            if (!success) {
              notice.notify({
                title: this.$t('preferences.keybindings.errors.saveFailed'),
                type: 'error',
                message: this.$t('preferences.keybindings.errors.saveError')
              })
            }
          })
    ...
    handleDuplicateShortcut (id, accelerator) {
      notice.notify({
        title: this.$t('preferences.keybindings.errors.shortcutInUse'),
        type: 'warning',
        message: this.$t('preferences.keybindings.errors.shortcutConflict')
      })
    },
