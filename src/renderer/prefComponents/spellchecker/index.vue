<template>
  <div class="pref-spellchecker">
    ...
    <compound>
      <template #head>
        <bool
          :description="$t('preferences.spellchecker.enable')"
      ...
      <template #children>
        <bool
          :description="$t('preferences.spellchecker.hideUnderline')"
        ...
        <bool
          v-show="isOsx"
          :description="$t('preferences.spellchecker.autoDetect')"
          :bool="true"
          :disable="true"
        ></bool>
        <cur-select
    ...
    <div v-if="isOsx && spellcheckerEnabled" class="description">
      {{ $t('preferences.spellchecker.osxDetectNote') }}
    </div>

    <div v-if="!isOsx && spellcheckerEnabled">
      <h6 class="title">{{ $t('preferences.spellchecker.customDictionary.title') }}</h6>
      <div class="description">{{ $t('preferences.spellchecker.customDictionary.edit') }}</div>
      <el-table
        :data="wordsInCustomDictionary"
        :empty-text="$t('preferences.spellchecker.customDictionary.noWords')"
        style="width: 100%"
      >
        ...
        <el-table-column fixed="right" :label="$t('common.options')" width="90">
          <template slot-scope="scope">
            <el-button @click="handleDeleteClick(scope.row)" type="text" size="small" :title="$t('common.delete')">
    ...
    handleSpellcheckerLanguage (languageCode) {
      this.ensureDictLanguage(languageCode)
        ...
        .catch(error => {
          log.error(error)
          notice.notify({
            title: this.$t('preferences.spellchecker.errors.switchFailed'),
            type: 'error',
        ...
        ipcRenderer.invoke('mt::spellchecker-remove-word', selectedItem.word)
          .then(success => {
            if (success) {
              ...
              notice.notify({
                title: this.$t('preferences.spellchecker.errors.removeFailed'),
                type: 'error',
                message: this.$t('preferences.spellchecker.errors.saveFailed')
              })
            }
          })
