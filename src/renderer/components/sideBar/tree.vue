  ...
  <div class="tree-view">
    ...
    <div class="opened-files">
      <div class="title">
        ...
        <span class="default-cursor text-overflow" @click.stop="toggleOpenedFiles()">{{ $t('components.sideBar.tree.openedFiles') }}</span>
        <a href="javascript:;" @click.stop="saveAll(false)" :title="$t('components.sideBar.tree.saveAll')">
          <svg class="icon" aria-hidden="true">
            <use xlink:href="#icon-save-all"></use>
          </svg>
        </a>
        <a href="javascript:;" @click.stop="saveAll(true)" :title="$t('components.sideBar.tree.closeAll')">
          <svg class="icon" aria-hidden="true">
    ...
    >
      ...
      <div class="tree-wrapper" v-show="showDirectories">
        ...
        <div class="empty-project" v-if="projectTree.files.length === 0 && projectTree.folders.length === 0">
          <span>{{ $t('components.sideBar.tree.emptyProject') }}</span>
          <a href="javascript:;" @click.stop="createFile">{{ $t('components.sideBar.tree.createFile') }}</a>
        </div>
      </div>
    </div>
    <div v-else class="open-project">
      <div class="centered-group">
        ...
        <button class="button-primary" @click="openFolder">
          {{ $t('components.sideBar.tree.openFolder') }}
        </button>
      </div>
    </div>
  </div>
